...
private:
	...
	std::map<string, std::shared_ptr<FPTreeNode>> children;
...
public:
	...
	std::shared_ptr<FPTreeNode> get_child(string value) {
		std::map<string, std::shared_ptr<FPTreeNode>>::iterator child_location =
			children.find(value);
