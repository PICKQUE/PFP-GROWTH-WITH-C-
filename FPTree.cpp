...
bool operator==(const Pattern& lhs, const Pattern& rhs) {
	return lhs.pattern_set == rhs.pattern_set;
}

Pattern operator-(const Pattern & lhs, const Pattern & rhs)
{
	Pattern temp(lhs);
	for (auto str : rhs) {
		auto it = std::find(temp.begin(),temp.end(), str);
		if (it!= temp.end()) {
			temp.erase(it);
		}
	}
...
void FPTree::build_support_list(const std::map<string, Item>& support_map) {
...
//int -> string
std::list<std::list<string>> FPTree::read_file(std::ifstream& fp) {
	std::list<std::list<string>> result;
	std::string line;
	string read_item;
	while (std::getline(fp, line)) {
		std::stringstream ss(line);
		result.push_back(std::list<string>());
		while (ss >> read_item) {
...
void FPTree::count_frequent_items(
	const std::list<std::list<string>>& transactions) {
...
void FPTree::build_tree(const std::list<std::list<string>>& transactions) {
	for (auto& transaction : transactions) {
		auto transaction_copy = std::list<string>(transaction);
		// Remove aqueles que nao tem o suporte minimo
		auto new_end = std::remove_if(transaction_copy.begin(),
			transaction_copy.end(), [&](const string& item) {
			return support_map[item].count < minimum_support;
		});
		transaction_copy.erase(new_end, transaction_copy.end());
		transaction_copy.sort([&](const string& a, const string& b) {
...
void FPTree::add_transaction(const std::list<string>& transaction) {
	auto current_node = root;
...
std::list<Pattern> FPTree::multi_path_patterns(const std::list<string>& prefix) {
...
#pragma omp parallel for if(launch_threads) num_threads(num_threads)
	for (auto it = support_list.rbegin(); it < support_list.rend(); it++) {
		...
		auto beta = std::list<string>(prefix);
...
std::list<Pattern> FPTree::add_all_prefix_combinations(FPTreeNodePtr current,
	...
	std::list<Pattern> result;
	auto itemset = std::list<string>(prefix);
	if (current == nullptr) {
		return result;
	}
	if (current->is_root()) {
...
void FPTree::build_fp_tree(const std::list<std::list<string>>& transactions) {
	count_frequent_items(transactions);
	build_tree(transactions);
}

std::list<Pattern> FPTree::fpgrowth(const std::list<string>& prefix) {
	std::list<Pattern> result;
	if (single_pathed) {
